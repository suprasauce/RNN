...
class rnn:
    def __init__(self, num_input: int, num_hidden: int, num_output: int, alpha, device):
        self.device = device
        self.alpha = alpha
        ...
        self.temp2_out = torch.zeros((self.num_hidden, self.num_input)).to(self.device)
        self.clip_value = 1.     
    ...
    def forward(self, X):
        ...
        for t in range(timesteps):
            # x_t[X[t]] = 1.
            c_t, h_t, f_t, i_t, o_gate_t, h_node_t = self.lstm_1.get_ct_ht(X[t].reshape((self.num_input, 1)), dict['c1_timesteps'][t-1].reshape((self.num_hidden, 1))
    ...
    def backward(self, X, Y, vals: dict):
        ...
        for t in range(timesteps-1, timesteps-2 , -1):
        
            y_hat_y = vals['o_timesteps'][t].reshape((self.num_output, 1)).clone().detach()
            # y_hat_y[Y[t]] -= 1.0
            y_hat_y = y_hat_y - Y.reshape((self.num_output, 1))
            ...
            for i in range(t, max(-1, t - self.truncate - 1), -1):
                ...
                main_c_delta += vals['o1_gate_timesteps'][i]*(1-torch.tanh(vals['c1_timesteps'][i])**2)*main_delta

                output_gate_delta1 = main_delta*torch.tanh(vals['c1_timesteps'][i])*vals['o1_gate_timesteps'][i]*(1-vals['o1_gate_timesteps'][i])
                forget_gate_delta1 = main_c_delta*vals['c1_timesteps'][i-1]*vals['f1_gate_timesteps'][i]*(1-vals['f1_gate_timesteps'][i])
                input_gate_delta1 = main_c_delta*vals['h1_node_timesteps'][i]*vals['i1_gate_timesteps'][i]*(1-vals['i1_gate_timesteps'][i])
                input_node_delta1 = main_c_delta*vals['i1_gate_timesteps'][i]*(1-vals['h1_node_timesteps'][i]**2)
                ...
                main_c_delta *= vals['f1_gate_timesteps'][i]
                
                torch.outer(output_gate_delta1, X[i], out = self.temp2_out)
                dict['dlstm']['dw_ox'] += self.temp2_out
                torch.outer(forget_gate_delta1, X[i], out = self.temp2_out)
                dict['dlstm']['dw_fx'] += self.temp2_out
                torch.outer(input_gate_delta1, X[i], out = self.temp2_out)
                dict['dlstm']['dw_ix'] += self.temp2_out
                torch.outer(input_node_delta1, X[i], out = self.temp2_out)
                dict['dlstm']['dw_hx'] += self.temp2_out

                # weights connected to previous time step hidden nodes

                torch.outer(output_gate_delta1, vals['h1_timesteps'][i-1], out = self.temp_out)
                dict['dlstm']['dw_oh'] += self.temp_out
                torch.outer(forget_gate_delta1, vals['h1_timesteps'][i-1], out = self.temp_out)
                dict['dlstm']['dw_fh'] += self.temp_out
                torch.outer(input_gate_delta1, vals['h1_timesteps'][i-1], out = self.temp_out)
                dict['dlstm']['dw_ih'] +=self.temp_out
                torch.outer(input_node_delta1, vals['h1_timesteps'][i-1], out = self.temp_out)
    ...
    def total_loss_of_one_context(self, Y, o_timesteps):
        # loss = torch.zeros((self.num_output, ))
        # for i in range(o_timesteps.shape[0]):
        loss = -1.*Y*torch.log(o_timesteps[-1])
        return torch.sum(loss) / len(Y)